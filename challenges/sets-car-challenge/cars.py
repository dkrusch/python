...
for make in makes:
    makes_and_colors[make[1]] = {}
    for model in models:
        if model[2] == make[0]:
            makes_and_colors[make[1]][model[1]] = []
            for available_color in available_car_colors:
                if available_color[0] == model[0]:
                    for color in colors:
                        if available_color[1] == color[0]:
                            makes_and_colors[make[1]][model[1]].append(color[1])


for key, attribute in makes_and_colors.items():
    print(f"{key}")
    print(f"------------------")
    for car, colors in attribute.items():
        print(f"{car} available in {', '.join(colors)}.")
    print("")
